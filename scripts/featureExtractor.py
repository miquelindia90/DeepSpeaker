...
def mfsc(
    ...
    D = np.abs(S)
    param = librosa.feature.melspectrogram(
        S=D, sr=sfr, n_mels=n_mels, fmin=lowfreq, fmax=highfreq, norm=None
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Extract Features. Looks for .wav files and extract Features"
    )
    parser.add_argument(
        "--audioFilesList",
        "-i",
