...
class HeadAttention(nn.Module):
    def __init__(
        self, encoder_size, heads_number, mask_prob=0.25, attentionSmoothing=False
    ):
    ...
    def __maskAttention(self, attention_score, mask_value=-float("inf")):
        mask = (
            torch.cuda.FloatTensor(attention_score.size()).random_(self.mask_prob) > 0
        )
...
def innerKeyValueAttention(query, key, value):
    ...
    scores = torch.diagonal(
        torch.matmul(key, query) / math.sqrt(d_k), dim1=-2, dim2=-1
    ).view(value.size(0), value.size(1), value.size(2))
...
class DoubleMHA(nn.Module):
    ...
    def getAlignments(self, x):
        ...
        headAlignments = self.headsAttention(
            utteranceRepresentation.view(
                utteranceRepresentation.size(0), self.heads_number, self.heads_size
            )
        )[1]
